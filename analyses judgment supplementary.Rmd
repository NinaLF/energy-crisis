---
title: "analyses judgment supplementary"
date: "`r Sys.Date()`"
output:
  html_document:
    theme: lumen
...
data.accuracy.pattern <- read.csv("wave 2 and both waves/data.supp/accuracy.patterns.csv")

data.accuracy.long.total <- read.csv("wave1 results and R/data.supp/data.accuracy.long.total.csv")
...
data.accuracy.w2.long.total <- read.csv("wave 2 and both waves/data.supp/data.accuracy.w2.long.total.csv")
...
#### long format
data.accuracy.waves.long <- read.csv("wave 2 and both waves/data.supp/data.accuracy.waves.long.csv")
...
```{r}
data.accuracy.waves.long.total  <- read.csv("wave 2 and both waves/data.supp/data.accuracy.waves.long.total.csv")
...
data.accuracy.test.freq <- read.csv("wave1 results and R/data.supp/data.accuracy.frequency.csv")
...
```{r, include=FALSE}
### both waves 
data.accuracy.waves.long.total$impact <- factor(data.accuracy.waves.long.total$behavior,
                                                levels=c("dishwasher", "dryer", "fridge", "hairdryer",
                                                         "heating", "light", "shower", "washing"),
                                                label=c("low", "high","low","low",
                                                        "high","low", "high", "low"))
...
#data.accuracy.waves.long.total_clean <-   data.accuracy.waves.long.total[!c( data.accuracy.waves.long.total$m %in% data.accuracy.pattern$m), ]
data.accuracy.waves.long.total_clean$impact <- factor(data.accuracy.waves.long.total_clean$behavior,
                                                levels=c("dishwasher", "dryer", "fridge", "hairdryer",
                                                         "heating", "light", "shower", "washing"),
                                                label=c("low", "high","low","low",
                                                        "high","low", "high", "low"))
...
model.impact.2 <- lmer(log.mis.estimation ~impact*wave + country + (1|m) + (1|behavior), data.accuracy.waves.long.total_clean2,   
  contrasts = list(
    impact = "contr.sum",
    wave = "contr.sum") )
...
model.error.waves.impact2 <- lmer(error ~impact*wave +numeracy.f*wave + country + (1|m) + (1|behavior), data.accuracy.waves.long.total_clean2.l)
