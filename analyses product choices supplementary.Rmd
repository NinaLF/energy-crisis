---
title: "analyses product choices supplementary"
date: "`r Sys.Date()`"
output:
  html_document:
    theme: lumen
...
knitr::opts_chunk$set(class.source = "CSSoutput",
                      ...
                      fig.align='center', fig.show='asis',  fig.width = 16, fig.height = 13,
                      size='footnotesize')
...
data.product.waves.long <- read.csv("wave 2 and both waves/data.supp/data.product.waves.long.csv")
...
data.everything <- read.csv("wave1 results and R/data.supp/data.everything.csv")
...
```{r}
### energy data
data.energy.plz.total.id <- read.csv( "wave 2 and both waves/data.supp/data.energy.plz.total.id.csv", dec=".")

mean(data.energy.plz.total.id$price.diff) + 3*sd(data.energy.plz.total.id$price.diff)
# mean +- 3* sd = 21.05042

data.energy.plz.total.id.exc <- data.energy.plz.total.id %>% filter(price.diff<= 22.21745 )
...
#### Simple slopes 
```{r}
#simple_slopes(model.choice.price.diff.prereg_clean)
#simple_slopes(model.choice.price.diff.prereg_clean, levels=list(rel_price_difference=c(-1,0,1, 'sstest'), wave=c("Wave 1", "Wave 2", 'sstest')) )

 #marginaleffects(model.choice.price.diff.prereg_clean,  variables = "rel_price_difference", by= "wave")
 #slopes(model.choice.price.diff.prereg_clean, variables = "rel_price_difference", by= "wave")
```
...
Anova(model.choice.price.diff.prereg.2, type=2)
```
