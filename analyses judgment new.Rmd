...
accuracy.waves.impact <- groupwiseMean(mis.estimation ~impact + wave + m,
data= data.accuracy.waves.long.total2.l)
t.test(data.accuracy.waves.long.total2.l$mis.estimation ~ data.accuracy.waves.long.total2.l$wave, paired=TRUE)

aov.estimation.bias <- anova_test(
  data = accuracy.waves.impact, dv = Mean, wid = m,
  within =c( wave, impact))
get_anova_table(aov.estimation.bias)
...
```{r}
accuracy.waves.impact.t
data.accuracy.waves.long.total_clean2$mis.estimation.log <- log10(data.accuracy.waves.long.total_clean2$mis.estimation)

t.test(data.accuracy.waves.long.total_clean2$mis.estimation ~ data.accuracy.waves.long.total_clean2$wave, paired=TRUE)

accuracy.waves.impact <- groupwiseMean(mis.estimation.log ~impact + wave + m,
data= data.accuracy.waves.long.total_clean2)
...
model.impact <- lmer(Mean ~impact*wave + (1|m), accuracy.waves.impact,   
  contrasts = list(
    impact = "contr.sum",
    wave = "contr.sum") )
Anova(model.impact, type=3, test.statistic="F")
...
accuracy.waves.error.num <- groupwiseMean(error ~ numeracy.f + wave  + impact +  m,
data= data.accuracy.waves.long.total_clean2.l)
t.test(accuracy.waves.error.num$Mean ~ accuracy.waves.error.num$wave, paired=TRUE)
...
model.error.waves.impact<- lmer(error ~numeracy.f*wave +wave*impact + country + (1|m), data.accuracy.waves.long.total_clean2.l,   
  contrasts = list(
    numeracy.f = "contr.sum",
    wave = "contr.sum",
    impact= "contr.sum",
    country = "contr.sum"))
Anova(model.error.waves.impact, type=3, test.statistic="F")
effectsize::eta_squared(model.error.waves.impact)
...
```{r}
pred_wave.impact <- ggemmeans(model.error.waves.impact, c("wave", "impact")) 
...
```{r}
pred_num.impact <- ggemmeans(model.error.waves.impact, c("wave", "numeracy.f")) 
...
```{r}
pred_num.impact.c <- ggemmeans(model.error.waves.impact, c("wave", "numeracy.f", "country")) 
...
pred_everything <- ggemmeans(model.error.waves.impact, c("wave", "numeracy.f", "impact", "country")) 
...
```{r}
## ch
data.accuracy.waves.long.total_clean2.ch <- data.accuracy.waves.long.total_clean2.l %>% filter(country=="Switzerland")

model.error.waves.impact.ch <- lmer(error ~ numeracy.f*wave +wave*impact  + (1|m), data.accuracy.waves.long.total_clean2.ch,   
  contrasts = list(
    numeracy.f = "contr.sum",
    wave = "contr.sum",
    impact= "contr.sum") )
Anova(model.error.waves.impact.ch, type=3, test.statistic="F")
effectsize::eta_squared(model.error.waves.impact.ch)
...
### Plot Switzerland
```{r}
plot_model(model.error.waves.impact.ch, type="pred", terms=c("wave","numeracy.f", "impact"),
...
```{r}
data.accuracy.waves.long.total_clean2.de <- data.accuracy.waves.long.total_clean2.l %>% filter(!country=="Switzerland")

model.error.waves.impact.de <- lmer(error ~ numeracy.f*wave +wave*impact + (1|m), data.accuracy.waves.long.total_clean2.de,   
  contrasts = list(
    numeracy.f = "contr.sum",
    wave = "contr.sum",
    impact= "contr.sum") )
tab_model(model.error.waves.impact.de)
 Anova(model.error.waves.impact.de, type=3, test.statistic="F")
effectsize::eta_squared(model.error.waves.impact.de)
...
plot_model(model.error.waves.impact.de, type="pred", terms=c("wave","numeracy.f", "impact"),
colors=c("#15A000", "#0070C0") ) + ylim(0.4,0.9) +
